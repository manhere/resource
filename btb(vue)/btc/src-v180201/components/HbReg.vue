<template>
  <section>
    ...
    <section class="btbinfo">
        ...
        <template v-else>
          <p><strong>2</strong>BTC</p>
          <p class="price">价值约{{unreginfo.receiveRedbagDetailInfo ? unreginfo.receiveRedbagDetailInfo.mayPoint :'0'}}</p>
        </template>
    ...
    <section class="asset">
        <div class="makeloading">5人正在领取，先生成钱包地址将优先获得红包</div>
    </section>
    <div class="pop" v-if="reg">
        <div class="register">
            <h4>恭喜您</h4>
            <p class="txt">获得<span>{{unreginfo.receiveRedbagDetailInfo ? unreginfo.receiveRedbagDetailInfo.mayPoint :'0'}}BTC</span>，填写手机号生成钱包地址</p>
...
export default {
  ...
  async created(){
    ...
    axios.post('/api/openRedbag').then(res => {
        console.log(res)
  ...
  methods:{
    ...
    gerHb(){
        ...
        if(this.phoneStatus && this.codeStatus && this.agree){
          ...
          axios.post('/api/createWalletAddress',params).then(res => {
              console.log(res);
              if(res.data.code == 0){
    ...
    getCode(){
      if(this.phone){
        var params = new URLSearchParams();
        params.append('phone', this.phone);
        axios.post('/api/code',params).then(res => {
            console.log(res);
            var to = 0;
            if(res.data.code == 0){
              // to = res.data.msg
