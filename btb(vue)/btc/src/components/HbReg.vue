<template>
  <section style="padding-bottom:1.2rem;">
    ...
    <section class="btbinfo">
        ...
        <template v-else>
          <p><strong>{{unreginfo.point}}</strong>{{unreginfo.receiveRedbagDetailInfo ? unreginfo.receiveRedbagDetailInfo.btcType :''}}</p>
          <p class="price">价值约{{unreginfo ? unreginfo.pointPrice :'0'}}元</p>
        </template>
    ...
    <div class="sponsor">
        <p>数字币提供技术支持</p>
        <p>www.shuzibi.com/123</p>
    </div>
    <section class="asset">
        <div class="makeloading"><template v-if="timeout">{{unreginfo.receiveRedbagDetailInfo ? unreginfo.receiveRedbagDetailInfo.hasCount:''}}人正在领取还剩{{unreginfo.receiveRedbagDetailInfo ? unreginfo.receiveRedbagDetailInfo.remainCount:''}}个红包，<br>先生成钱包地址将优先获得红包</template><template v-else>{{unreginfo.receiveRedbagDetailInfo ? unreginfo.receiveRedbagDetailInfo.hasCount:''}}人正在领取，先生成钱包地址将优先获得红包</template></div>
    </section>
    <div class="pop" v-if="reg">
        <div class="register">
            <h4>恭喜您</h4>
            <p class="txt">获得<span>{{unreginfo ? unreginfo.point :'0'}}{{unreginfo.receiveRedbagDetailInfo ? unreginfo.receiveRedbagDetailInfo.btcType:''}}</span>，填写手机号生成钱包地址</p>
